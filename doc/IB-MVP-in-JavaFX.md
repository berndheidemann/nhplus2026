...
<p style="text-align: justify;">Der zur jeweiligen View gehörende Presenter wird aus der FXML-Datei geladen und automatisch instanziiert. Im MainWindowController reicht dafür der Aufruf <code>loader.load()</code> — die in der FXML-Datei deklarierte <code>fx:controller="...AllPatientController"</code>-Angabe sorgt dafür, dass der FXMLLoader die passende Presenter-Klasse selbst instanziiert. Dabei wird automatisch die Methode <code>initialize()</code> aus der Klasse AllPatientController aufgerufen. Diese muss nur explizit aufgerufen werden, sofern sie vom Presenter überschrieben wird, ihr Aufruf also Parameter erfordert.</p>

<p style="text-align: justify;">Falls der aufrufende Code anschließend mit dem geladenen Presenter weiterarbeiten muss — zum Beispiel, um ihm Daten oder einen Verweis auf das aufrufende Fenster zu übergeben — kann der Presenter über den Loader geholt werden:</p>

```
NewTreatmentController controller = loader.getController();
```

<p style="text-align: justify;">So geschieht es im AllTreatmentController beim Öffnen des Behandlungs-Dialogs.</p>
